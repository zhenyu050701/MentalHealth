...
def show_analytics():
    ...
    try:
        ...
        fig = px.pie(gender_counts, values='Count', names='Gender', hole=0.3)
        st.plotly_chart(fig, use_container_width=True)

        st.subheader("📊 Stress vs. Sleep Quality")
        fig = px.bar(df, x='Stress Level', y='Sleep Quality', color='Gender', barmode='group')
        st.plotly_chart(fig, use_container_width=True)

        st.subheader("🔍 % of Anxious Users with Low Social Support")
        anxious_users = df[df['Anxiety Level'] > 3]
        support_counts = anxious_users['Social Support'].value_counts().reset_index()
        support_counts.columns = ['Social Support', 'Count']
        fig = px.pie(support_counts, values='Count', names='Social Support', hole=0.3)
        st.plotly_chart(fig, use_container_width=True)

        st.subheader("⚠️ Anxiety vs. Self-Harm Cases")
        fig = px.scatter(df, x='Anxiety Level', y='Self Harm Risk', color='Gender')
        st.plotly_chart(fig, use_container_width=True)
